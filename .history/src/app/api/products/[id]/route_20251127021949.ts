...
export async function PUT(
  ...
  try {
    ...
    if (newImages.length > 0) {
      uploadedNewImages = await CloudinaryAPI.UpdateImages(newImages, id);
    }
    ...
    productToUpload.images = [...existingImages, ...uploadedNewImages];
    ...
    if (removeList > 0) {
      //Aqui iría el codigo para borrar las imagenes de la base de datos
    ...
    if (data.images && data.images.length > 0) {
      const uploaded = await CloudinaryAPI.UploadImages(data.images, finalSlug);
